    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column {
            Row(
                ...
                SearchBarForComponent(
                    ...
                    onQueryChange = { query ->
                        ...
                        totalPages.value = maxOf(1, (filteredCpuCoolers.size / itemsPerPage) + if (filteredCpuCoolers.size % itemsPerPage == 0) 0 else 1)
            ...
            Surface(
                ...
                if (filteredCpuCoolers.isEmpty()) {
                    // Show empty state when no data is available
                    Text(
                        text = "No CPU coolers match your filters.",
                        color = Color.Gray
                    )
                } else {
                    CpuCoolerList(
    ...
    LazyColumn(
        ...
        items(currentPageCoolers) { cooler ->
            ...
            ComponentCard(
                ...
                priceConvert = convertPrice(cooler.price,context),
                title = cooler.name,
                context = context,
                details = """
                            Name: ${cooler.name}
