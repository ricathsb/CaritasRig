...
fun CpuScreen(navController: NavController) {
    ...
    val coreCountRange = remember { mutableStateOf(0f..30f) }
    val coreClockRange = remember { mutableStateOf(0f..10f) }
    val boostClockRange = remember { mutableStateOf(0f..10f) }
        ...
        if (showFilterDialog) {
            FilterDialog(
                ...
                onApply = { coreCount, coreClock, boostClock, brands ->
                    ...
                    Log.d("FilterDialog", "Filtered Processors: $filteredProcessors")
                }
            )
        }
    }
